...
<body>
  <main>
    ...
    <form action="/InscripcionesEVG/index.php?controlador=inscripcionesActividades&accion=cInscribirAlumnos" method="POST" class="form-container">
        ...
        <?php if (isset($dataToView["data"]['alumnos'])): ?>
          ...
          if (!empty($nombresInscritos)) {
            foreach ($nombresInscritos as $index => $nombreInscrito):
              $alumnoInscrito = current(array_filter(
                $dataToView["data"]['alumnos'],
