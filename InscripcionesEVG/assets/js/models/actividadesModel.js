class ActividadesModel {
	constructor() {
		this.baseUrl = "/InscripcionesEVG/index.php?controlador=actividades";
	}

	async mostrarActividades(idMomento) {
		try {
			const response = await fetch(
				`${this.baseUrl}&accion=cMostrarActividadesporIdMomento&momento=${idMomento}`,
			);
	...
	async insertarActividad(formData) {
		try {
			const response = await fetch(
				`${this.baseUrl}&accion=cInsertarActividad`,
				{
					method: "POST",
					body: formData,
				},
	...
	async eliminarActividad(idActividad) {
		try {
			const response = await fetch(
				`${this.baseUrl}&accion=cEliminarActividad&idActividad=${idActividad}`,
			);
			return await response.json();
		} catch (error) {
