class ActividadesController {
    ...
    initializeEventListeners() {
        ...
        if (formActividad) {
            formActividad.addEventListener('submit', (e) => this.handleInsertarActividad(e));
        }
        ...
        if (formEditarActividad) {
            formEditarActividad.addEventListener('submit', (e) => this.handleEditarActividad(e));
        }
        ...
        if (btnConfirmarEliminar) {
            btnConfirmarEliminar.addEventListener('click', () => this.handleEliminarActividad());
        }
    ...
    validarInsertarActividad(formData) {
        ...
        if (isNaN(maxParticipantes) || parseInt(maxParticipantes) <= 0) {
            throw new Error('El número máximo de participantes debe ser un número positivo');
        }
    ...
    async handleEliminarActividad() {
        ...
        try {
            const resultado = await this.model.eliminarActividad(this.idActividadAEliminar);
            if (resultado.success) {
